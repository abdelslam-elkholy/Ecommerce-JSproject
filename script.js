...
let searchFunc = () => {
  ...
  if (searchValue !== "") {
    ...
    filtered.map((el) => {
      ...
      element.innerHTML = `
         <button class="dropdown-item" type="button"  onclick="getIdOfProduct(${el.id})" >
           <a href="./product.html" style="text-decoration: none;color:black;
           font-weight:bold"> ${el.title.slice(0, 35)}</a>
         </button>`;
...
// Change Products Number In Cart Icon 
let changCartNum = () => {
...
let displayCartProducts = async() => {
  ...
  choosen.map(el => {
    ...
    product.innerHTML = 
    `
    <img src="${el.image}" class=" border-0 p-2"  alt="" style="height:10rem!important; max-width:15rem"
    ${el.title} image">
          <div class="card-body d-flex flex-column justify-content-between">
...
// Show one Product In A single Page
